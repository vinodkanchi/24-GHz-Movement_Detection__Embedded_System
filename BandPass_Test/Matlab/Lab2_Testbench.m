...
delete(priorPorts);
PSoC=serial('COM3', 'BaudRate', 9600, 'InputBufferSize', 14000);
fopen(PSoC);
...
while(flg_data_avai == 0)
       ...
       if PSoC.BytesAvailable >= 2048
             ...
             rx_data_adc = fread(PSoC,1024,'uint16')*((4.5)/(2^16));
             ...
             subplot(2,1,2)
             plot([0:1023]*(2000/1024),1/length(rx_data_adc)*20*log10(abs(fft(rx_data_adc))));
             title('FFT -  Matlab');
             ylabel('mag/dB');
             xlabel('f/Hz');
